...
exports.default = function (state, actions) {
	...
	var match;
	for (var rule in Config.modules['radio-stream'].rules) {
		match = (0, _helpers.syntax)(state.sentence, Config.modules['radio-stream'].rules[rule]);
		if (match) break;
	}

	if (match) { // vérifié !
		if (state.debug) info('Intent radio-stream, Syntax: true');
		state.isIntent = true;
